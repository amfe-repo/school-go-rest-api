...
func GetEnrollCoursesStudentsHandler(w http.ResponseWriter, r *http.Request) {
	var enrollCourses []utils.StudentEnrollCourses
	...
	if res.Error != nil || res.RowsAffected < 1 {
		...
		for counter := range enrollCourses {
			db.DB.Model(&enrollCourses[counter]).Association("Student").Find(&enrollCourses[counter].Student)
...
func GetEnrollCoursesStudentHandler(w http.ResponseWriter, r *http.Request) {
	var enrollCourse utils.StudentEnrollCourses
	params := mux.Vars(r)

	response := &httpop.Response{}

	if response.ValidateError(db.DB.First(&enrollCourse, params["id"]), w, "enroll course not found") {
		db.DB.Model(&enrollCourse).Association("Student").Find(&enrollCourse.Student)
...
func PostEnrollCoursesStudentHandler(w http.ResponseWriter, r *http.Request) {
	var course utils.StudentEnrollCourses
...
func UpdateEnrollCoursesStudentHandler(w http.ResponseWriter, r *http.Request) {
	var course, newCourse utils.StudentEnrollCourses
	params := mux.Vars(r)
...
func DeleteEnrollCoursesStudentHandler(w http.ResponseWriter, r *http.Request) {
